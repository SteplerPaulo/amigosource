...
class CustomFormHelper extends FormHelper {
	function input($fieldName, $options = array()) {
		...
		if(!isset($options['div'])){
			$options['div'] = $this->_inputDefaults['div']['class'];
		}
		$modelKey = $this->model();
		$fieldKey = $this->field();
		...
		switch ($type) {
			...
			case 'url':
			case 'number':
				$input = $this->customType($fieldName, $type, $options);
			break;
			case 'datetime':
	...
	function customType($fieldName, $type='text',$options = array()) {
		$type_options = array('type'=>$type);
		if($type=='email'){
			$type_options ['pattern'] = '[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$';
		}
